...
export const ARRAY_SUMMARY_CODE_LANGUAGES: Record<string, string[]> = {
  java: [
    ...
    '2. 对撞双指针: int l = 0, r = n - 1; while (l <= r) { int lsq = nums[l] * nums[l], rsq = nums[r] * nums[r]; ... }',
    '3. 滑动窗口: for (int r = 0; r < n; r++) { sum += nums[r]; while (sum >= target) { ... sum -= nums[l++]; } }',
    '4. 二分查找: int l = 0, r = n - 1; while (l <= r) { int m = l + (r - l) / 2; if (nums[m] == target) return m; }',
    '5. 一维前缀和: prefix[i + 1] = prefix[i] + arr[i]; // sum[L..R] = prefix[R+1] - prefix[L]',
    '6. 二维前缀和: prefix[i+1][j+1] = grid[i][j] + prefix[i][j+1] + prefix[i+1][j] - prefix[i][j];',
    '7. 四边界模拟: while (num <= n * n) { /* top++, right--, bottom--, left++ */ }',
  ],
  cpp: [
    '// 数组解题范式速览 (C++)',
    '1. 快慢双指针: int slow = 0; for (int fast = 0; fast < n; fast++) if (cond) nums[slow++] = nums[fast];',
  ...
  python: [
    ...
    '# 2. 对撞双指针: l, r = 0, n - 1; while l <= r: lsq, rsq = nums[l]**2, nums[r]**2; ...',
    '# 3. 滑动窗口: for r in range(n): sum_val += nums[r]; while sum_val >= target: ... sum_val -= nums[l]; l += 1',
    '# 4. 二分查找: l, r = 0, n - 1; while l <= r: m = (l + r) // 2; if nums[m] == target: return m',
    '# 5. 一维前缀和: prefix[i + 1] = prefix[i] + arr[i]',
    '# 6. 二维前缀和: prefix[i+1][j+1] = grid[i][j] + prefix[i][j+1] + prefix[i+1][j] - prefix[i][j]',
    '# 7. 四边界模拟: while num <= n * n: # top += 1, right -= 1, bottom -= 1, left += 1',
  ],
  javascript: [
    '// 数组解题范式速览 (JavaScript)',
    '// 1. 快慢双指针: let slow = 0; for (let fast = 0; fast < n; fast++) if (cond) nums[slow++] = nums[fast];',
    '// 2. 对撞双指针: let l = 0, r = n - 1; while (l <= r) { const lsq = nums[l]**2, rsq = nums[r]**2; ... }',
    '// 3. 滑动窗口: for (let r = 0; r < n; r++) { sum += nums[r]; while (sum >= target) { ... sum -= nums[l++]; } }',
    '// 4. 二分查找: let l = 0, r = n - 1; while (l <= r) { const m = Math.floor((l + r) / 2); if (nums[m] === target) return m; }',
    '// 5. 一维前缀和: prefix[i + 1] = prefix[i] + arr[i];',
    '// 6. 二维前缀和: prefix[i+1][j+1] = grid[i][j] + prefix[i][j+1] + prefix[i+1][j] - prefix[i][j];',
    '// 7. 四边界模拟: while (num <= n * n) { /* top++, right--, bottom--, left++ */ }',
  ],
