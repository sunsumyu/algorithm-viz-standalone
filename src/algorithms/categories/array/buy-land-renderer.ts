  ...
  protected renderStep(step: BLStep): void {
    ...
    if (this.logContainer) {
      const logs = this.steps.slice(0, this.currentIndex + 1).map((st, idx) => {
        let bg = st.status === 'done' ? '#f0fdf4' : st.status === 'update-best' ? '#eff6ff' : '#f8fafc';
        let color = st.status === 'done' ? '#15803d' : st.status === 'update-best' ? '#1d4ed8' : '#334155';
        let border = st.status === 'done' ? '#bbf7d0' : st.status === 'update-best' ? '#bfdbfe' : '#e2e8f0';
