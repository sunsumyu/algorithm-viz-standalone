  ...
  protected renderStep(step: TSStep): void {
    ...
    if (this.logContainer) {
      const logs = this.steps.slice(0, this.currentIndex + 1).map((st, idx) => {
        let bg =
          st.phase === 'symmetric' ? '#f0fdf4' : st.phase === 'asymmetric' ? '#fff1f2' : '#eff6ff';
        let color =
          st.phase === 'symmetric' ? '#15803d' : st.phase === 'asymmetric' ? '#e11d48' : '#1d4ed8';
        let border =
          st.phase === 'symmetric' ? '#bbf7d0' : st.phase === 'asymmetric' ? '#fecdd3' : '#bfdbfe';
