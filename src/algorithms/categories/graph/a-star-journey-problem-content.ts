...
export const A_STAR_JOURNEY_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  python: [
    '# A* 算法网格寻路 (Python 标准实现)',
    '# 核心原理: f(n) = g(n) + h(n)，g 为真实耗费，h 为启发式曼哈顿距离',
    '# 小根堆每次取出综合估价 f 最小的节点，若 h(n) <= h*(n) (可采纳性)，首次出队目标点即最优解',
    'import heapq',
    '',
    ...
    '        if (r, c) == (tr, tc):',
    '            return g  # 首次触达目标点，由可采纳性定理保证为全局最优',
    '        if (r, c) in visited:',
    '            continue',
    '        visited.add((r, c))',
    '        ',
  ...
  javascript: [
    ...
    ' * 综合代价 f(n) = g(n) + h(n)，优先队列小根堆维护 Open 表',
    ' */',
    ...
    '  gScore[sr][sc] = 0;',
    '  ',
    '  // 使用简易数组模拟优先队列，生产环境建议使用 Binary Heap',
