...
export const KM_ALGORITHM_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '        for (int i = 1; i <= n; i++) {',
    '            lx[i] = -1000000000000000000L;',
    '            for (int j = 1; j <= n; j++) lx[i] = Math.max(lx[i], weight[i][j]);',
    '        }',
    '        for (int i = 1; i <= n; i++) {',
  ...
  javascript: [
    ...
    '          if (!this.visY[j]) d = Math.min(d, this.slack[j]);',
    '        }',
    '        for (let j = 0; j < this.n; j++) {',
    '          if (this.visX[j]) this.lx[j] -= d;',
    '          if (this.visY[j]) this.ly[j] += d;',
    '          else this.slack[j] -= d;',
    '        }',
    '      } ',
    '    }',
