...
export const HLD_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    }',
    '    ',
    '    // 3. 跳重链获取树上路径对应的连续 DFN 区间集',
    '    public static List<int[]> getPathSegments(int u, int v) {',
    '        List<int[]> segments = new ArrayList<>();',
    '        while (top[u] != top[v]) {',
    '            if (depth[top[u]] < depth[top[v]]) {',
    '                int tmp = u; u = v; v = tmp;',
    '            }',
    '            segments.add(new int[]{dfn[top[u]], dfn[u]});',
    '            u = parent[top[u]];',
    '        }',
    '        if (depth[u] > depth[v]) {',
    '            int tmp = u; u = v; v = tmp;',
    '        }',
    '        segments.add(new int[]{dfn[u], dfn[v]});',
    '        return segments;',
    '    }',
    '}',
  ],
  python: [
    '# 树链剖分算法 (Python 标准实现)',
    '# 核心依据: 2 次 DFS 划分重链，将树上任意路径转化为至多 O(log n) 个连续区间',
    '# 时间复杂度: 预处理 O(N)，路径查询/修改 O(log^2 N)',
    'class HLD:',
    ...
    '                self.dfs2(v, v)',
    '                ',
    '    def get_path_segments(self, u: int, v: int) -> list[tuple[int, int]]:',
    '        # 跳重链向上汇合至 LCA，记录所有沿途的连续 DFN 闭区间',
    '        segments = []',
    '        while self.top[u] != self.top[v]:',
    '            if self.depth[self.top[u]] < self.depth[self.top[v]]:',
    '                u, v = v, u',
    '            segments.append((self.dfn[self.top[u]], self.dfn[u]))',
    '            u = self.parent[self.top[u]]',
    '        if self.depth[u] > self.depth[v]:',
    '            u, v = v, u',
    '        segments.append((self.dfn[u], self.dfn[v]))',
    '        return segments',
  ],
  javascript: [
    '/**',
    ' * 树链剖分 (JavaScript 版)',
    ' * 2 次 DFS 确定重儿子与链头，转化为连续 DFN 区间',
    ' */',
    'class HLD {',
    '  constructor(n) {',
    '    this.n = n;',
    ...
    '        [u, v] = [v, u];',
    '      }',
    '      segments.push([this.dfn[this.top[u]], this.dfn[u]]);',
    '      u = this.parent[this.top[u]];',
    '    }',
    '    if (this.depth[u] > this.depth[v]) {',
    '      [u, v] = [v, u];',
    '    }',
    '    segments.push([this.dfn[u], this.dfn[v]]);',
    '    return segments;',
    '  }',
    '}',
  ],
