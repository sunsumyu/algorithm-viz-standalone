...
export const TARJAN_SCC_DAG_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    ',
    '    public static void tarjan(int u, ArrayList<ArrayList<Integer>> graph) {',
    '        dfn[u] = low[u] = ++timer;',
    '        stack.push(u);',
    '        inStack[u] = true;',
    '        ',
    ...
    '        }',
    '    }',
    '    public static ArrayList<ArrayList<Integer>> buildDAG(int n, ArrayList<ArrayList<Integer>> graph) {',
  ...
  python: [
    ...
    '',
    'def build_dag(n: int, scc_count: int, scc_id: list[int], graph: list[list[int]]) -> list[list[int]]:',
  ...
  javascript: [
    ...
    '}',
    '',
    'function buildDAG(n, sccCount, sccId, graph) {',
    '  const dag = Array.from({ length: sccCount + 1 }, () => []);',
    '  for (let u = 1; u <= n; u++) {',
