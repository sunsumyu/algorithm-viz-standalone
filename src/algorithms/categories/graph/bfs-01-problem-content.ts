...
export const BFS_01_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  python: [
    '# 0-1 BFS 双端队列求最短路 (Python 标准实现)',
    '# 核心原理: 边权为 0 放队头 (appendleft)，边权为 1 放队尾 (append)',
    '# 时间复杂度: O(V + E) 或 O(M * N)，完全避免优先队列的 O(E log V)',
    'from collections import deque',
    '',
    '# 1. 网格模型: 消除障碍物的最小数目 (LeetCode 2290)',
    ...
    '',
    '# 2. 通用图模型: 0-1 BFS 最短路',
    'def bfs_01_graph(n: int, edges: list[tuple[int, int, int]], start: int) -> list[int]:',
  ...
  javascript: [
    ...
    ' * 保持队列单调性与两段性，以严格 O(V + E) 代替 Dijkstra 堆优化',
    ' */',
    ...
    '  ',
    '  while (dq.length > 0) {',
    '    const [x, y] = dq.shift(); // popleft 队头出队',
    '    if (x === m - 1 && y === n - 1) return dist[x][y];',
    '    ',
    ...
    '',
    '// 通用图 0-1 最短路封装',
    'function bfs01Graph(n, edges, start) {',
