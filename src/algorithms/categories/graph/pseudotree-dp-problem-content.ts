...
export const PSEUDOTREE_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    public static ArrayList<ArrayList<int[]>> adj;',
    ...
    '        if (cutEdgeId == -1) return 0;',
    '        treeDP(rootU, -1);',
    '        long ans1 = dp[rootU][0];',
    '        treeDP(rootV, -1);',
    '        long ans2 = dp[rootV][0];',
  ...
  python: [
    '# 基环树 DP (Python 标准实现)',
    '# 核心依据: DFS 找环，断开环边 (u, v)，分别强制不选 u 和不选 v 跑两次树形 DP',
    '# 时间复杂度: O(N)，空间复杂度: O(N)',
    'class PseudotreeDP:',
    ...
    '        self.find_circle(start, -1)',
    '        if self.cut_edge == -1: return 0',
    '        self.tree_dp(self.root_u, -1)',
    '        ans1 = self.dp[self.root_u][0]',
    '        self.tree_dp(self.root_v, -1)',
    '        ans2 = self.dp[self.root_v][0]',
    '        return max(ans1, ans2)',
  ],
  javascript: [
    '/**',
    ...
    '  treeDP(u, edgeId) {',
    '    this.dp[u][0] = 0;',
    '    this.dp[u][1] = this.w[u];',
    '    for (const [v, eid] of this.adj[u]) {',
    '      if (eid === edgeId || eid === this.cutEdgeId) continue;',
    '      this.treeDP(v, eid);',
    ...
    '  ',
    '  solve(start) {',
    '    this.findCircle(start, -1);',
    '    if (this.cutEdgeId === -1) return 0;',
    '    this.treeDP(this.rootU, -1);',
    '    const ans1 = this.dp[this.rootU][0];',
    '    this.treeDP(this.rootV, -1);',
    '    const ans2 = this.dp[this.rootV][0];',
    '    return Math.max(ans1, ans2);',
    '  }',
    '}',
  ],
...
export const PSEUDOTREE_DP_CODE_LANGUAGES = PSEUDOTREE_CODE_LANGUAGES;
export const PSEUDOTREE_DP_PROBLEM_HTML = PSEUDOTREE_PROBLEM_HTML;
export const PSEUDOTREE_DP_ANALYSIS_HTML = PSEUDOTREE_ANALYSIS_HTML;
