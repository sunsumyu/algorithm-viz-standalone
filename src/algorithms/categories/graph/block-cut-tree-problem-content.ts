...
export const BLOCK_CUT_TREE_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    public static ArrayList<ArrayList<Integer>> origAdj, treeAdj;',
    ...
    '                    while (true) {',
    '                        int top = st.pop();',
    '                        treeAdj.get(top).add(square);',
    '                        treeAdj.get(square).add(top);',
    '                        if (top == v) break;',
    '                    }',
    '                    treeAdj.get(u).add(square);',
    '                    treeAdj.get(square).add(u);',
    '                }',
    '            } else {',
  ...
  python: [
    '# 广义圆方树 (Python 标准实现)',
    '# 核心依据: Tarjan 求点双连通分量 (v-BCC)，每个点双对应一个方点，圆方相间连边成树',
  ...
  javascript: [
    ...
    '          this.squareCnt += 1;',
    '          const sq = this.squareCnt;',
    '          while (true) {',
    '            const top = this.st.pop();',
    '            this.treeAdj[top].push(sq);',
    '            this.treeAdj[sq].push(top);',
    '            if (top === v) break;',
    '          }',
    '          this.treeAdj[u].push(sq);',
    '          this.treeAdj[sq].push(u);',
    '        }',
    '      } else {',
