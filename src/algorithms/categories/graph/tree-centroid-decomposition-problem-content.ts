...
export const TREE_CENTROID_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    }',
    '    ',
    '    public static List<Integer> dists = new ArrayList<>();',
    '    ',
    '    public static void getDists(int u, int p, int d) {',
    '        dists.add(d);',
    '        for (int[] edge : adj.get(u)) {',
  ...
  python: [
    '# 树上重心点分治算法 (Python 标准实现)',
    '# 核心依据: 寻找树重心保证递归层数 O(log n)，按重心统计路径并容斥去重',
    ...
    '            self.root = u',
    '            ',
    '    def get_dists(self, u: int, p: int, d: int):',
    '        self.dists.append(d)',
    '        for v, w in self.adj[u]:',
    '            if v != p and not self.vis[v]:',
    '                self.get_dists(v, u, d + w)',
    '                ',
    '    def solve(self, u: int):',
    '        self.vis[u] = True',
    '        for v, w in self.adj[u]:',
    '            if not self.vis[v]:',
    '                self.max_part = float("inf")',
    '                self.total_nodes = self.sz[v]',
    '                self.get_centroid(v, 0)',
    '                self.solve(self.root)',
  ],
  javascript: [
    '/**',
    ' * 树上点分治 (JavaScript 版)',
    ' * 递归寻找重心，将路径转化为过重心与不过重心',
    ' */',
    'class CentroidDecomposition {',
    '  constructor(n) {',
    '    this.n = n;',
    ...
    '      if (!this.vis[v]) {',
    '        this.maxPart = Infinity;',
    '        this.totalNodes = this.sz[v];',
    '        this.getCentroid(v, 0);',
