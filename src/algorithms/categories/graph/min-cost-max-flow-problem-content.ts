...
export const MCMF_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    }',
    '    ',
    '    public static int[] getMinCostMaxFlow(int s, int t, int n) {',
    '        int maxFlow = 0, minCost = 0;',
    '        while (spfa(s, t, n)) {',
    '            int pushed = Integer.MAX_VALUE;',
    '            for (int u = t; u != s; u = preNode[u]) {',
    '                Edge e = graph[preNode[u]].get(preEdge[u]);',
    '                pushed = Math.min(pushed, e.cap - e.flow);',
    '            }',
    '            for (int u = t; u != s; u = preNode[u]) {',
    '                Edge e = graph[preNode[u]].get(preEdge[u]);',
    '                e.flow += pushed;',
    '                graph[u].get(e.rev).flow -= pushed;',
    '            }',
    '            maxFlow += pushed;',
    '            minCost += pushed * dist[t];',
    '        }',
  ...
  python: [
    ...
    '        ',
    '    def get_min_cost_max_flow(self) -> tuple[int, int]:',
    ...
    '            while u != self.s:',
    '                p = self.pre_node[u]',
    '                e_idx = self.pre_edge[u]',
    '                v, cap, flow, cost, rev = self.graph[p][e_idx]',
    ...
    '                p = self.pre_node[u]',
    '                e_idx = self.pre_edge[u]',
    '                rev = self.graph[p][e_idx][4]',
    '                self.graph[p][e_idx][2] += pushed',
    '                self.graph[u][rev][2] -= pushed',
    '                u = p',
    '            max_flow += pushed',
    '            min_cost += pushed * self.dist[self.t]',
  ...
  javascript: [
    ...
    '    while (this.spfa()) {',
    '      let pushed = Infinity;',
    '      for (let u = this.t; u !== this.s; u = this.preNode[u]) {',
    '        const e = this.graph[this.preNode[u]][this.preEdge[u]];',
    '        pushed = Math.min(pushed, e.cap - e.flow);',
    '      }',
    '      for (let u = this.t; u !== this.s; u = this.preNode[u]) {',
    '        const e = this.graph[this.preNode[u]][this.preEdge[u]];',
    '        e.flow += pushed;',
    '        this.graph[u][e.rev].flow -= pushed;',
    '      }',
    '      maxFlow += pushed;',
    '      minCost += pushed * this.dist[this.t];',
    '    }',
    '    return { maxFlow, minCost };',
    '  }',
    '}',
