...
export const STABLE_MARRIAGE_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '',
    '/**',
    ' * 稳定婚姻匹配 (Stable Marriage Problem - Gale-Shapley 延迟接受算法)',
    ' * 核心：男士主动求婚、女士择优暂留，必定收敛且不存在不稳定阻碍对 (Blocking Pair)',
    ' * 时间复杂度: O(N^2)，空间复杂度: O(N^2)',
    ' */',
    'public class Code01_GaleShapley {',
    '    public static int n;',
    '    public static int[][] menPref;   // menPref[m][k]: 男士 m 第 k 偏好的女士',
    '    public static int[][] womenRank; // womenRank[w][m]: 女士 w 对男士 m 的喜爱排名 (值越小越喜欢)',
    '    public static int[] husband, wife, nextPropose;',
    '    ',
    '    public static void init(int totalN, int[][] mPref, int[][] wPref) {',
    '        n = totalN;',
    '        menPref = mPref;',
    '        womenRank = new int[n + 1][n + 1];',
    '        for (int w = 1; w <= n; w++) {',
    '            for (int rank = 0; rank < n; rank++) {',
    '                int m = wPref[w][rank];',
    '                womenRank[w][m] = rank; // 建立 O(1) 偏好查询查表',
    '            }',
    '        }',
    '        husband = new int[n + 1];',
    '        wife = new int[n + 1];',
    '        nextPropose = new int[n + 1];',
    '    }',
    '    ',
    ...
    '                    // 女士移情别恋新追求者 m，替换旧未婚夫 cur',
    '                    husband[w] = m;',
    '                    wife[m] = w;',
    '                    wife[cur] = 0;',
    '                    freeMen.offer(cur); // 旧未婚夫重获单身',
    '                } else {',
    '                    // 女士拒绝 m，m 依然单身并在后续尝试次佳心仪对象',
    '                    freeMen.offer(m);',
    '                }',
    '            }',
    '        }',
  ...
  python: [
    '# 稳定婚姻问题 (Gale-Shapley 延迟接受算法 / Python 标准实现)',
    '# 核心：男士依次向心仪女士求婚，女士保留当前最好并延迟接受',
    '# 时间复杂度: O(N^2)，空间复杂度: O(N^2)',
    'class GaleShapley:',
    ...
    '                self.women_rank[w][m] = rank # O(1) 查表判定女士喜好',
    '        self.husband = [0] * (n + 1)',
    '        self.wife = [0] * (n + 1)',
  ...
  javascript: [
    '/**',
    ' * 稳定婚姻问题 (Gale-Shapley 算法 / JavaScript 标准版)',
    ' * 核心：男士主动求婚、女士择优暂留，必定收敛且无不稳定阻碍对',
    ' * 时间复杂度: O(N^2)，空间复杂度: O(N^2)',
    ' */',
    'class GaleShapley {',
    ...
    '  solve() {',
    '    const freeMen = [];',
    '    for (let i = 1; i <= this.n; i++) freeMen.push(i);',
    '    ',
    '    while (freeMen.length > 0) {',
    '      const m = freeMen.shift();',
    '      const w = this.menPref[m][this.nextPropose[m]++];',
    '      ',
    '      if (this.husband[w] === 0) {',
    '        // 女士当前单身，直接接受',
    '        this.husband[w] = m;',
    '        this.wife[m] = w;',
    '      } else {',
    '        const curHusband = this.husband[w];',
    '        if (this.womenRank[w][m] < this.womenRank[w][curHusband]) {',
    '          // 女士更喜欢 m，移情别恋并抛弃当前未婚夫',
    '          this.husband[w] = m;',
    '          this.wife[m] = w;',
    '          this.wife[curHusband] = 0;',
    '          freeMen.push(curHusband); // 前未婚夫重新单身',
    '        } else {',
    '          // 拒绝求婚者 m',
    '          freeMen.push(m);',
    '        }',
