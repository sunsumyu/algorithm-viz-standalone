...
export const TWO_SAT_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    'import java.util.*;',
    '',
    '// 2-SAT 问题与 Tarjan 强连通分量判定 - 左程云 Class 078',
    '// 核心：子句 (u or v) 转化为蕴涵边 (~u -> v, ~v -> u)，Tarjan 缩点判定与拓扑赋值',
    ...
    '    }',
    '    ',
    '    // 添加子句 (u == valU) OR (v == valV)',
    '    public static void addClause(int u, int valU, int v, int valV) {',
    '        int nodeU = valU == 1 ? 2 * u - 1 : 2 * u;',
    '        int notU  = valU == 1 ? 2 * u : 2 * u - 1;',
    '        int nodeV = valV == 1 ? 2 * v - 1 : 2 * v;',
    '        int notV  = valV == 1 ? 2 * v : 2 * v - 1;',
    '        adj.get(notU).add(nodeV); // ~u -> v',
    '        adj.get(notV).add(nodeU); // ~v -> u',
    '    }',
    '    ',
    '    // Tarjan 求强连通分量 (SCC)',
    ...
    '    ',
    '    // 求解 2-SAT 并构造赋值解向量',
    '    public static boolean solve(int[] assignment) {',
    '        for (int i = 1; i <= 2 * n; i++) {',
    '            if (dfn[i] == 0) tarjan(i);',
    '        }',
    '        for (int i = 1; i <= n; i++) {',
    ...
    '                return false; // 同一变量真假在同一 SCC，无解矛盾',
    '            }',
    '            // scc 编号越小，拓扑序越靠后，优先赋真',
    '            assignment[i] = scc[trueNode] < scc[falseNode] ? 1 : 0;',
    '        }',
    '        return true;',
    '    }',
  ...
  python: [
    '# 2-SAT 问题与 Tarjan SCC 求解 (Python 3)',
    'class TwoSAT:',
    ...
    '            if self.dfn[i] == 0:',
    '                self.tarjan(i)',
    '        assignment = [0] * (self.n + 1)',
    '        for i in range(1, self.n + 1):',
    '            true_node = 2 * i - 1',
    '            false_node = 2 * i',
    '            if self.scc[true_node] == self.scc[false_node]:',
    '                return False, []',
    '            assignment[i] = 1 if self.scc[true_node] < self.scc[false_node] else 0',
    '        return True, assignment',
  ],
  javascript: [
    '// 2-SAT 问题与 Tarjan SCC 判定 (JavaScript ES6+)',
    'class TwoSAT {',
    '  constructor(n) {',
    '    this.n = n;',
    ...
    '  ',
    '  tarjan(u) {',
    '    this.dfn[u] = this.low[u] = ++this.dfnCnt;',
    '    this.stack.push(u);',
    '    this.inStack[u] = true;',
    ...
    '        this.inStack[top] = false;',
    '        this.scc[top] = this.sccCnt;',
    '        if (top === u) break;',
    '      }',
    '    }',
    '  }',
    ...
    '    }',
    '    const assignment = Array(this.n + 1).fill(0);',
    '    for (let i = 1; i <= this.n; i++) {',
    '      const trueNode = 2 * i - 1;',
    '      const falseNode = 2 * i;',
    '      if (this.scc[trueNode] === this.scc[falseNode]) {',
    '        return { satisfiable: false, assignment: [] };',
    '      }',
    '      assignment[i] = this.scc[trueNode] < this.scc[falseNode] ? 1 : 0;',
    '    }',
