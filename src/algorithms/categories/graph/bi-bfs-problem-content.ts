...
export const BI_BFS_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  python: [
    ...
    '    # 双向广搜 (Python 标准实现)',
    '    # 核心依据: 始终优先选择较小的集合向前扩散，搜索空间由 b^d 降低到 2*b^(d/2)',
    '    # 经典场景: LeetCode 127 单词接龙 / 八数码 / 最短路径快速汇聚',
    ...
    '            for nxt in get_neighbors(word):',
    '                if nxt in big_set:',
    '                    return step  # 双向波前相遇碰撞！',
    '                if nxt in words and nxt not in visited:',
    '                    next_set.add(nxt)',
    '                    visited.add(nxt)',
    '        ',
    '        # 核心优化：每次将节点数更少的一侧作为下一次的 small_set',
