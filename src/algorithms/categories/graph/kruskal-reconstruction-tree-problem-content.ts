...
export const KRUSKAL_TREE_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '                father[fu] = nodeCount;',
    '                father[fv] = nodeCount;',
    '                tree.get(nodeCount).add(fu);',
    '                tree.get(nodeCount).add(fv);',
    '            }',
    '        }',
    '    }',
    '    ',
    '    public static int queryBottleneck(int lcaNode) {',
    '        return val[lcaNode];',
    '    }',
    '}',
  ],
  python: [
    '# Kruskal 重构树算法 (Python 标准实现)',
    '# 核心原理：边权升序合并，新建虚点作为左右子树的父节点',
    '# 点权具有大根堆性质，任意两点 u, v 的路径瓶颈边权恰好等于 val[LCA(u, v)]',
    '# 时间复杂度: O(M log M + Q log N)，空间复杂度: O(N + M)',
    'class KruskalReconstructionTree:',
    ...
    '                ',
    '    def query_bottleneck(self, lca_node: int) -> int:',
    '        # 查询两点路径瓶颈（LCA 节点点权）',
    '        return self.val[lca_node]',
  ],
  javascript: [
    '/**',
    ' * Kruskal 重构树算法 (JavaScript 版)',
    ' * 边权升序合并，点权单调，LCA 对应路径瓶颈',
    ' */',
    'class KruskalReconstructionTree {',
    '  constructor(n) {',
    '    this.n = n;',
    ...
    '  }',
    '  ',
    '  queryBottleneck(lcaNode) {',
    '    return this.val[lcaNode];',
    '  }',
    '}',
  ],
};
...
export const KRUSKAL_RECONSTRUCTION_TREE_CODE_LANGUAGES = KRUSKAL_TREE_CODE_LANGUAGES;
export const KRUSKAL_RECONSTRUCTION_TREE_PROBLEM_HTML = KRUSKAL_TREE_PROBLEM_HTML;
export const KRUSKAL_RECONSTRUCTION_TREE_ANALYSIS_HTML = KRUSKAL_TREE_ANALYSIS_HTML;
