...
export const KONIG_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    ',
    '    public static void init(int numL, int numR) {',
    '        n = numL;',
    '        m = numR;',
    '        adj = new ArrayList<>();',
    ...
    '        matchR = new int[m + 1];',
    '        vis = new boolean[m + 1];',
    '        visL = new boolean[n + 1];',
    '        visR = new boolean[m + 1];',
    '    }',
    '    ',
    ...
    '        int ans = 0;',
    '        Arrays.fill(matchL, 0);',
    '        Arrays.fill(matchR, 0);',
    '        for (int i = 1; i <= n; i++) {',
    ...
    '    }',
    '    ',
    '    public static void alternatingDfs(int u) {',
    '        visL[u] = true;',
    '        for (int v : adj.get(u)) {',
    ...
    '    }',
    '    ',
    '    public static List<String> getMinVertexCover() {',
    ...
    '        for (int u = 1; u <= n; u++) {',
    '            if (!visL[u]) cover.add("L" + u);',
    '        }',
    '        for (int v = 1; v <= m; v++) {',
    '            if (visR[v]) cover.add("R" + v);',
    '        }',
    '        return cover;',
    '    }',
    '    ',
    '    public static List<String> getMaxIndependentSet() {',
    '        List<String> independent = new ArrayList<>();',
    '        for (int u = 1; u <= n; u++) {',
    '            if (visL[u]) independent.add("L" + u);',
    '        }',
    '        for (int v = 1; v <= m; v++) {',
    '            if (!visR[v]) independent.add("R" + v);',
    '        }',
    '        return independent;',
    '    }',
    '}',
  ...
  python: [
    ...
    '        ans = 0',
    '        self.match_l = [0] * (self.n + 1)',
    '        self.match_r = [0] * (self.m + 1)',
    '        for i in range(1, self.n + 1):',
    '            self.vis = [False] * (self.m + 1)',
    '            if self.dfs(i):',
    '                ans += 1',
    '        return ans',
    '        ',
    '    def alternating_dfs(self, u: int):',
    '        self.vis_l[u] = True',
    '        for v in self.adj[u]:',
    '            if not self.vis_r[v] and v != self.match_l[u]:',
    '                self.vis_r[v] = True',
    '                if self.match_r[v] != 0 and not self.vis_l[self.match_r[v]]:',
    '                    self.alternating_dfs(self.match_r[v])',
    '                    ',
    '    def get_min_vertex_cover(self) -> list[str]:',
    '        self.vis_l = [False] * (self.n + 1)',
    '        self.vis_r = [False] * (self.m + 1)',
    '        for u in range(1, self.n + 1):',
    '            if self.match_l[u] == 0 and not self.vis_l[u]:',
    '                self.alternating_dfs(u)',
    '        cover = []',
    '        for u in range(1, self.n + 1):',
    '            if not self.vis_l[u]:',
    '                cover.append(f"L{u}")',
    '        for v in range(1, self.m + 1):',
    '            if self.vis_r[v]:',
    '                cover.append(f"R{v}")',
    '        return cover',
  ],
  javascript: [
    ...
    '    this.visL[u] = true;',
    '    for (const v of this.adj[u]) {',
    '      if (!this.visR[v] && v !== this.matchL[u]) {',
    '        this.visR[v] = true;',
    '        if (this.matchR[v] !== 0 && !this.visL[this.matchR[v]]) {',
    '          this.alternatingDfs(this.matchR[v]);',
    '        }',
    '      }',
    '    }',
    '  }',
    '  ',
    '  getMinVertexCover() {',
    '    this.visL.fill(false);',
    '    this.visR.fill(false);',
    '    for (let u = 1; u <= this.n; u++) {',
    '      if (this.matchL[u] === 0 && !this.visL[u]) {',
    '        this.alternatingDfs(u);',
    '      }',
    '    }',
    '    const cover = [];',
    '    for (let u = 1; u <= this.n; u++) {',
    '      if (!this.visL[u]) cover.push(`L${u}`);',
    '    }',
    '    for (let v = 1; v <= this.m; v++) {',
    '      if (this.visR[v]) cover.push(`R${v}`);',
    '    }',
    '    return cover;',
    '  }',
    '}',
