...
export const TOPO_DP_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  python: [
    '# 拓扑排序与 DAG 动态规划 / 最长关键路径 (Python 标准实现)',
    '# 核心原理: DAG 无环无后效性，按拓扑序状态转移: dp[v] = max(dp[v], dp[u] + w)',
    '# 时间复杂度: O(V + E)，空间复杂度: O(V + E)',
    'from collections import deque',
    '',
    '# 1. 并行课程 III 模型: 节点带耗时权重 (LeetCode 2050)',
  ...
  javascript: [
    '/**',
    ' * 拓扑排序与 DAG 关键路径 / 动态规划 (JavaScript 版)',
    ' * 遵循拓扑序推进，严格消除状态后效性，时间复杂度 O(V + E)',
    ' */',
    ...
    '',
    '// 通用 DAG 边权最长路求解',
    'function dagLongestPath(n, edges) {',
