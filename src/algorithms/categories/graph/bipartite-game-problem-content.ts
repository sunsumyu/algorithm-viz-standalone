...
export const BIPARTITE_GAME_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '        matchRight = new int[m + 1];',
    '        vis = new boolean[Math.max(n, m) + 1];',
    '        canBeUnmatched = new boolean[n + 1];',
    '    }',
    '    ',
    ...
    '    }',
    '    ',
    '    public static boolean dfsHungar(int u) {',
    '        for (int v : adj.get(u)) {',
    '            if (vis[v]) continue;',
    '            vis[v] = true;',
    ...
    '    }',
    '    ',
    '    public static void dfsAlternate(int u) {',
    '        canBeUnmatched[u] = true;',
    '        for (int v : adj.get(u)) {',
    '            if (!vis[v] && matchRight[v] > 0) {',
    '                vis[v] = true;',
    ...
    '    }',
    '    ',
    '    public static List<Integer> getWinningStartNodes() {',
    '        for (int i = 1; i <= n; i++) {',
    ...
    '        }',
    '        ',
    '        List<Integer> winNodes = new ArrayList<>();',
    '        for (int i = 1; i <= n; i++) {',
    '            if (matchLeft[i] > 0 && !canBeUnmatched[i]) {',
    '                winNodes.add(i);',
    '            }',
    '        }',
    '        return winNodes;',
  ...
  python: [
    ...
    '        self.vis = [False] * (max(n, m) + 1)',
    '        self.can_be_unmatched = [False] * (n + 1)',
    '        ',
    '    def add_edge(self, u: int, v: int):',
    '        self.adj[u].append(v)',
    '        ',
    '    def dfs_hungar(self, u: int) -> bool:',
    '        for v in self.adj[u]:',
    '            if self.vis[v]:',
    '                continue',
    '            self.vis[v] = True',
    '            if self.match_right[v] == 0 or self.dfs_hungar(self.match_right[v]):',
    '                self.match_right[v] = u',
    '                self.match_left[u] = v',
    '                return True',
    '        return False',
    '        ',
    '    def dfs_alternate(self, u: int):',
    '        self.can_be_unmatched[u] = True',
    '        for v in self.adj[u]:',
    '            if not self.vis[v] and self.match_right[v] > 0:',
    '                self.vis[v] = True',
    '                self.dfs_alternate(self.match_right[v])',
    '                ',
    '    def get_winning_start_nodes(self) -> list[int]:',
    '        for i in range(1, self.n + 1):',
    '            self.vis = [False] * (max(self.n, self.m) + 1)',
    '            self.dfs_hungar(i)',
    '            ',
    '        self.vis = [False] * (max(self.n, self.m) + 1)',
    '        for i in range(1, self.n + 1):',
    '            if self.match_left[i] == 0:',
    '                self.dfs_alternate(i)',
    '                ',
    '        winning_nodes = []',
    '        for i in range(1, self.n + 1):',
    '            if self.match_left[i] > 0 and not self.can_be_unmatched[i]:',
  ...
  javascript: [
    ...
    '    this.vis = Array(Math.max(n, m) + 1).fill(false);',
    ...
    '      this.vis[v] = true;',
    '      if (this.matchRight[v] === 0 || this.dfsHungar(this.matchRight[v])) {',
    '        this.matchRight[v] = u;',
    '        this.matchLeft[u] = v;',
    '        return true;',
    '      }',
    ...
    '  ',
    '  dfsAlternate(u) {',
    '    this.canBeUnmatched[u] = true;',
    '    for (const v of this.adj[u]) {',
    '      if (!this.vis[v] && this.matchRight[v] > 0) {',
    '        this.vis[v] = true;',
    '        this.dfsAlternate(this.matchRight[v]);',
    '      }',
    '    }',
    '  }',
    '  ',
    '  getWinningStartNodes() {',
    ...
    '    const winningNodes = [];',
    '    for (let i = 1; i <= this.n; i++) {',
    '      if (this.matchLeft[i] > 0 && !this.canBeUnmatched[i]) {',
    '        winningNodes.push(i);',
    '      }',
    '    }',
    '    return winningNodes;',
