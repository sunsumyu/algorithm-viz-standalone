...
export const TREE_PATH_INTERSECT_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '',
    '// 树上路径相交判定 (洛谷 P3398 仓鼠找 sugar)',
    '// 核心定理：路径 (u1, v1) 与 (u2, v2) 相交 <=> LCA(u1,v1) 在 (u2,v2) 上 或 LCA(u2,v2) 在 (u1,v1) 上',
    ...
    '',
    '    // 判断点 x 是否在树上路径 (u, v) 上',
    '    public static boolean isNodeOnPath(int x, int u, int v) {',
    '        return getDist(u, x) + getDist(x, v) == getDist(u, v);',
    '    }',
    '',
    '    // 判定路径 (u1, v1) 与 (u2, v2) 是否相交',
    '    public static boolean isPathsIntersect(int u1, int v1, int u2, int v2) {',
    '        int lca1 = getLCA(u1, v1);',
    '        int lca2 = getLCA(u2, v2);',
    '        boolean on1 = isNodeOnPath(lca1, u2, v2);',
    '        boolean on2 = isNodeOnPath(lca2, u1, v1);',
    '        return on1 || on2;',
    '    }',
    '',
    '    // 树上初始化与查询入口',
    '    public static boolean solve(int numNodes, int[][] edges, int u1, int v1, int u2, int v2) {',
    '        n = numNodes;',
    '        adj = new ArrayList<>();',
    ...
    '        dfs(1, 0, 1);',
    '',
    '        return isPathsIntersect(u1, v1, u2, v2);',
    '    }',
    '}',
  ],
  python: [
    '# 树上路径相交判定 (Python 版)',
    'def is_paths_intersect(n: int, edges: list[tuple[int, int]], u1: int, v1: int, u2: int, v2: int) -> bool:',
    ...
    '    ',
    '    def is_node_on_path(x: int, u: int, v: int) -> bool:',
    '        return get_dist(u, x) + get_dist(x, v) == get_dist(u, v)',
    '    ',
    '    lca1 = get_lca(u1, v1)',
    '    lca2 = get_lca(u2, v2)',
    '    return is_node_on_path(lca1, u2, v2) or is_node_on_path(lca2, u1, v1)',
  ],
  javascript: [
    '// 树上路径相交判定 (JavaScript 版)',
    'function isPathsIntersect(n, edges, u1, v1, u2, v2) {',
    '  const adj = Array.from({ length: n + 1 }, () => []);',
    '  for (const [u, v] of edges) {',
    '    adj[u].push(v);',
    '    adj[v].push(u);',
    '  }',
    '  const up = Array.from({ length: n + 1 }, () => new Array(20).fill(0));',
    ...
    '  }',
    '  ',
    '  function isNodeOnPath(x, u, v) {',
    '    return getDist(u, x) + getDist(x, v) === getDist(u, v);',
    '  }',
    '  ',
    '  const lca1 = getLCA(u1, v1);',
    '  const lca2 = getLCA(u2, v2);',
    '  return isNodeOnPath(lca1, u2, v2) || isNodeOnPath(lca2, u1, v1);',
    '}',
  ],
};
