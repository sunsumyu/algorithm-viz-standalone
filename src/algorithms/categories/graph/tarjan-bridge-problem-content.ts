...
export const TARJAN_BRIDGE_CODE_LANGUAGES: Record<string, string[]> = {
  ...
  java: [
    ...
    '    ',
    '    public static void tarjan(int u, int father, ArrayList<ArrayList<Integer>> graph) {',
    '        dfn[u] = low[u] = ++timer;',
    '        int children = 0;',
    '        ',
    ...
    '    }',
    '    ',
    '    public static void solve(int n, ArrayList<ArrayList<Integer>> graph) {',
    '        timer = 0;',
    '        dfn = new int[n + 1];',
    '        low = new int[n + 1];',
    ...
    '        bridges.clear();',
    '        for (int i = 1; i <= n; i++) {',
    '            if (dfn[i] == 0) tarjan(i, 0, graph);',
    '        }',
    '    }',
    '}',
  ],
  python: [
    '# Tarjan 无向图割点与割边 (桥) 算法',
    '# 时间复杂度: O(V + E)，空间复杂度: O(V + E)',
    '# 核心依据：low[v] >= dfn[u] 为割点；low[v] > dfn[u] 为桥边',
  ...
  javascript: [
    ...
    ' * @returns {{ isCut: boolean[], bridges: number[][] }}',
    ' */',
