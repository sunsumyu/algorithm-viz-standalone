...
export const COINS_FROM_PILES_STAGE1_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    ...
    'int dfs(int i, int remK, const vector<vector<int>>& piles) {',
    '    if (i == piles.size() || remK == 0) return 0;',
    '    // 分支 0: 当前栈拿 0 枚',
    '    int maxVal = dfs(i + 1, remK, piles);',
    '    // 分支 c: 当前栈从顶向下连续拿 c 枚',
    '    int sum = 0;',
  ...
  java: [
    ...
    '    public static int dfs(int i, int remK, List<List<Integer>> piles) {',
    '        if (i == piles.size() || remK == 0) return 0;',
    '        int maxVal = dfs(i + 1, remK, piles);',
    '        int sum = 0;',
  ...
  python: [
    'def dfs_coins(i: int, rem_k: int, piles: list[list[int]]) -> int:',
    '    """阶段 1: 暴力递归"""',
    '    if i == len(piles) or rem_k == 0:',
    '        return 0',
    '    max_val = dfs_coins(i + 1, rem_k, piles)',
    '    s = 0',
  ...
  javascript: [
    '// 阶段 1: 暴力递归',
    'export function dfsCoins(i, remK, piles) {',
    '  if (i === piles.length || remK === 0) return 0;',
    '  let maxVal = dfsCoins(i + 1, remK, piles);',
    '  let sum = 0;',
...
export const COINS_FROM_PILES_STAGE2_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    ...
    'int dfsMemo(int i, int remK, const vector<vector<int>>& piles, vector<vector<int>>& memo) {',
    '    if (i == piles.size() || remK == 0) return 0;',
    '    if (memo[i][remK] != -1) return memo[i][remK]; // 命中缓存',
  ...
  java: [
    ...
    '    public static int dfsMemo(int i, int remK, List<List<Integer>> piles, int[][] memo) {',
    '        if (i == piles.size() || remK == 0) return 0;',
  ...
  python: [
    'def coins_memo(piles: list[list[int]], k: int) -> int:',
    '    """阶段 2: 记忆化搜索"""',
...
export const BUYING_HAY_STAGE1_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    ...
    '// 阶段 1: 暴力递归',
    'const int INF = 1e9;',
    'int dfs(int i, int remH, int n, const vector<int>& cost, const vector<int>& val) {',
    '    if (remH <= 0) return 0; // 已满足磅数需求',
    '    if (i == n) return INF;  // 供货商已耗尽但仍未满足',
    '    // 分支 0: 不买当前第 i 家',
    '    int p1 = dfs(i + 1, remH, n, cost, val);',
    '    // 分支 1: 买 1 包第 i 家 (完全背包可继续买第 i 家)',
    '    int p2 = dfs(i, remH - val[i], n, cost, val) + cost[i];',
    '    return min(p1, p2);',
    '}',
  ],
  java: [
    ...
    '    public static int dfs(int i, int remH, int n, int[] cost, int[] val) {',
    '        if (remH <= 0) return 0;',
    '        if (i == n) return INF;',
    '        int p1 = dfs(i + 1, remH, n, cost, val);',
    '        int p2 = dfs(i, remH - val[i], n, cost, val) + cost[i];',
  ...
  python: [
    ...
    '    n = len(cost)',
    '    INF = float("inf")',
    '    def dfs(i: int, rem_h: int) -> int:',
    '        if rem_h <= 0:',
    '            return 0',
    '        if i == n:',
    '            return INF',
    '        p1 = dfs(i + 1, rem_h)',
    '        p2 = dfs(i, rem_h - val[i]) + cost[i]',
    '        return min(p1, p2)',
    '    return dfs(0, h)',
  ],
  javascript: [
...
export const BUYING_HAY_STAGE2_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    ...
    'const int INF = 1e9;',
    'int dfsMemo(int i, int remH, int n, const vector<int>& cost, const vector<int>& val, vector<vector<int>>& memo) {',
    '    if (remH <= 0) return 0;',
    '    if (i == n) return INF;',
    '    if (memo[i][remH] != -1) return memo[i][remH]; // 命中缓存',
    '    int p1 = dfsMemo(i + 1, remH, n, cost, val, memo);',
    '    int p2 = dfsMemo(i, remH - val[i], n, cost, val, memo) + cost[i];',
    '    return memo[i][remH] = min(p1, p2);',
    '}',
  ],
  java: [
    'package class074;',
    ...
    '    public static final int INF = 1_000_000_000;',
    '    public static int dfsMemo(int i, int remH, int n, int[] cost, int[] val, int[][] memo) {',
    '        if (remH <= 0) return 0;',
    '        if (i == n) return INF;',
    '        if (memo[i][remH] != -1) return memo[i][remH];',
    '        int p1 = dfsMemo(i + 1, remH, n, cost, val, memo);',
    '        int p2 = dfsMemo(i, remH - val[i], n, cost, val, memo) + cost[i];',
    '        return memo[i][remH] = Math.min(p1, p2);',
    '    }',
    '}',
  ],
  python: [
    'def buying_hay_stage2(h: int, cost: list[int], val: list[int]) -> int:',
    '    """阶段 2: 记忆化搜索"""',
    '    n = len(cost)',
    '    INF = float("inf")',
    ...
    '        p1 = dfs(i + 1, rem_h)',
    '        p2 = dfs(i, rem_h - val[i]) + cost[i]',
    '        memo[i][rem_h] = min(p1, p2)',
    '        return memo[i][rem_h]',
    '    return dfs(0, h)',
  ],
  javascript: [
    '// 阶段 2: 记忆化搜索',
    'export function buyingHayStage2(h, cost, val) {',
    '  const n = cost.length;',
    '  const INF = 1e9;',
