...
export const KNAPSACK_01_STAGE1_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    '#include <iostream>',
    '#include <vector>',
    '#include <algorithm>',
    ...
    'int dfs(int i, int remCap, int m, const vector<int>& cost, const vector<int>& val) {',
    '    if (i >= m || remCap <= 0) return 0;',
    '    // 分支 1: 不选当前物品',
    '    int p1 = dfs(i + 1, remCap, m, cost, val);',
    '    // 分支 2: 选入当前物品（消耗容量，推进至下一件物品）',
    '    int p2 = 0;',
    '    if (remCap >= cost[i]) {',
    '        p2 = dfs(i + 1, remCap - cost[i], m, cost, val) + val[i];',
    '    }',
    '    return max(p1, p2);',
    '}',
  ],
  java: [
    ...
    '    }',
    '    private static int dfs(int i, int remCap, int m, int[] cost, int[] val) {',
    '        if (i >= m || remCap <= 0) return 0;',
    '        int p1 = dfs(i + 1, remCap, m, cost, val);',
    '        int p2 = 0;',
    '        if (remCap >= cost[i]) {',
  ...
  python: [
    'def dfs_01(i: int, rem_cap: int, cost: list[int], val: list[int]) -> int:',
    '    """阶段 1: 01背包暴力递归"""',
    '    if i >= len(cost) or rem_cap <= 0:',
    '        return 0',
    '    p1 = dfs_01(i + 1, rem_cap, cost, val)',
    '    p2 = 0',
    '    if rem_cap >= cost[i]:',
    '        p2 = dfs_01(i + 1, rem_cap - cost[i], cost, val) + val[i]',
    '    return max(p1, p2)',
  ],
  javascript: [
    '// 阶段 1: 01背包暴力递归',
    'export function dfs01Knapsack(i, remCap, cost, val) {',
    '  if (i >= cost.length || remCap <= 0) return 0;',
    '  const p1 = dfs01Knapsack(i + 1, remCap, cost, val);',
    '  let p2 = 0;',
    '  if (remCap >= cost[i]) {',
    '    p2 = dfs01Knapsack(i + 1, remCap - cost[i], cost, val) + val[i];',
    '  }',
    '  return Math.max(p1, p2);',
    '}',
  ],
...
export const KNAPSACK_01_STAGE2_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    '#include <iostream>',
    '#include <vector>',
    '#include <algorithm>',
    ...
    'int dfsMemo(int i, int remCap, int m, const vector<int>& cost, const vector<int>& val, vector<vector<int>>& memo) {',
    '    if (i >= m || remCap <= 0) return 0;',
    '    if (memo[i][remCap] != -1) return memo[i][remCap]; // 命中缓存',
    '    int p1 = dfsMemo(i + 1, remCap, m, cost, val, memo);',
    '    int p2 = 0;',
    '    if (remCap >= cost[i]) {',
    '        p2 = dfsMemo(i + 1, remCap - cost[i], m, cost, val, memo) + val[i];',
    '    }',
  ...
  java: [
    ...
    '    public static int compute(int t, int m, int[] cost, int[] val) {',
    '        int[][] memo = new int[m][t + 1];',
    '        for (int[] row : memo) Arrays.fill(row, -1);',
    '        return dfsMemo(0, t, m, cost, val, memo);',
    '    }',
    '    private static int dfsMemo(int i, int remCap, int m, int[] cost, int[] val, int[][] memo) {',
    '        if (i >= m || remCap <= 0) return 0;',
  ...
  python: [
    'def knapsack_01_memo(t: int, m: int, cost: list[int], val: list[int]) -> int:',
    '    """阶段 2: 01背包记忆化搜索"""',
    ...
    '        p1 = dfs(i + 1, rem_cap)',
    '        p2 = dfs(i + 1, rem_cap - cost[i]) + val[i] if rem_cap >= cost[i] else 0',
  ...
  javascript: [
    '// 阶段 2: 01背包记忆化搜索',
    'export function knapsack01Memo(t, m, cost, val) {',
    '  const memo = Array.from({ length: m }, () => new Array(t + 1).fill(-1));',
    '  function dfs(i, remCap) {',
    '    if (i >= m || remCap <= 0) return 0;',
    '    if (memo[i][remCap] !== -1) return memo[i][remCap];',
    '    const p1 = dfs(i + 1, remCap);',
    '    const p2 = remCap >= cost[i] ? dfs(i + 1, remCap - cost[i]) + val[i] : 0;',
    '    return (memo[i][remCap] = Math.max(p1, p2));',
    '  }',
    '  return dfs(0, t);',
    '}',
...
export const KNAPSACK_01_STAGE3_CODE_LANGUAGES: Record<string, string[]> = {
  cpp: [
    '#include <iostream>',
    '#include <vector>',
    '#include <algorithm>',
    ...
    '',
    '// 阶段 3: 严格二维动态规划 01背包',
    '// dp[i][j]: 前 i 件物品在容量限制为 j 时的最大价值',
    'int compute01Knapsack2D(int t, int m, const vector<int>& cost, const vector<int>& val) {',
  ...
  java: [
    ...
    '                dp[i][j] = dp[i - 1][j];',
    '                if (j >= cost[i - 1]) {',
    '                    dp[i][j] = Math.max(dp[i][j], dp[i - 1][j - cost[i - 1]] + val[i - 1]);',
    '                }',
    '            }',
    '        }',
    '        return dp[m][t];',
