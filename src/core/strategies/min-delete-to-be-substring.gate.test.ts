...
describe('最少删除使成为子串 (MinDeleteToBeSubstring, Class068 题目4) 黄金基准 TDD 门禁测试', () => {
  const algoId = 'min-delete-to-be-substring';
  ...
  it('策略注册中心必须正确匹配 min-delete-to-be-substring', () => {
  ...
  it('Stage 1 递归必须构建合法 DFS 树、调用栈与足迹', () => {
    const model = AlgorithmModelRepository.getModel(algoId);
    const steps = UniversalStageEngine.generateStage1or2Steps(model, 4, 13, 'forward', false);
  ...
  it('Stage 2 记忆化搜索在遇到重叠子问题时必须能够记录 cache-hit 剪枝步骤', () => {
    const model = AlgorithmModelRepository.getModel(algoId);
    // 使用存在多个相同字符的用例 (s1='ab', s2='aab') 触发跨 j 的相同子状态记忆化命中
    const overlapModel = {
      ...model,
      defaultParams: { s1: 'ab', s2: 'aab' }
    };
    const steps = UniversalStageEngine.generateStage1or2Steps(overlapModel, 2, 3, 'forward', true);

    const hitSteps = steps.filter(s => s.type === 'cache-hit');
    expect(hitSteps.length, '记忆化阶段必须触发 cache-hit 剪枝').toBeGreaterThan(0);
  });

  it('Stage 3 严格二维表拓扑递推：Step 0 纯净 null 防御，生命周期完整且正确计算结果', () => {
    const model = AlgorithmModelRepository.getModel(algoId);
    const fSteps = UniversalStageEngine.generateStage3Steps(model, 4, 13, 'forward');
    const rSteps = UniversalStageEngine.generateStage3Steps(model, 4, 13, 'reverse');
    ...
    // 4. 最终答案断言: s1 = "abdf", s2 = "dfxxabYYabfzz" -> 最少删除 1 ('d')
    const fReturn = fSteps[fSteps.length - 1];
    expect(fReturn.type).toBe('return');
    expect(fReturn.val, 's1="abdf", s2="dfxxabYYabfzz" 顺推最少删除应为 1').toBe(1);

    const rReturn = rSteps[rSteps.length - 1];
    expect(rReturn.type).toBe('return');
    expect(rReturn.val, 's1="abdf", s2="dfxxabYYabfzz" 逆推最少删除应为 1').toBe(1);
  });

  it('Stage 4 一维空间压缩优化：Step 0 纯净 null，网格严格降维至一维，物理实体跳跃有效', () => {
    const model = AlgorithmModelRepository.getModel(algoId);
    const fSteps = UniversalStageEngine.generateStage4Steps(model, 4, 13, 'forward');
    const rSteps = UniversalStageEngine.generateStage4Steps(model, 4, 13, 'reverse');
    ...
    // 3. 结果正确性
    const fReturn = fSteps[fSteps.length - 1];
    expect(fReturn.val, 'Stage 4 顺推最终答案必须为 1').toBe(1);

    const rReturn = rSteps[rSteps.length - 1];
    expect(rReturn.val, 'Stage 4 逆推最终答案必须为 1').toBe(1);
  });
});
