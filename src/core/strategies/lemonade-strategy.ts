...
export class LemonadeStrategy extends BaseAlgorithmStrategy {
  constructor() {
    super('lemonade');
  }
  ...
  protected compileStage(
    ...
    const rawBills = options?.customInputs?.bills ?? model.defaultParams?.bills;
    const bills = this.parseSequence(rawBills, [5, 5, 5, 10, 20]);
